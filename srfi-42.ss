     ...
     (:do cc
          (let ((a arg1) (b arg2) (s arg3) (istop 0))
            (if (not (and (real? a) (real? b) (real? s)))
                (error ':real-range "arguments of :real-range are not real: ~s ~s ~s" a b s) )
    ...
    ((:dispatched cc var dispatch arg1 arg ...)
     (:do cc
          (let ((d dispatch) 
            ...
            (set! g (d args))
            (if (not (procedure? g))
                (error ':dispatched "unrecognized arguments in dispatching: ~s ~s" 
                       args 
                       (d '()) )))
          ((var (g empty)))
          (not (eq? var empty))
      ...
      (if g1
          (if g2 
              (if (null? args)
                  ...
                  (error ':dispatch-union "dispatching conflict: ~s ~s ~s" args (d1 '()) (d2 '())) )
              g1 )
          (if g2 g2 #f) ))))
...
(define :-dispatch
  (let ([dispatch (make-initial-:-dispatch)])
    (case-lambda
      [() dispatch]
      [(new-dispatch) (set! dispatch new-dispatch)])))
...
(define (:-dispatch-set! dispatch)
  (if (not (procedure? dispatch))
      (error ':-dispatch-set! "not a procedure: ~s" dispatch) )
  (:-dispatch dispatch) )
